...
def Team_2_page():
    st.title("Team_2 일정관리")

    # 예를 들어, 'Team_2'의 group_id가 1이이라면
    if "projects_Team_2" not in st.session_state:
        # group_id가 1인 프로젝트를 로드하여 세션 상태에 저장
        st.session_state["projects_Team_2"] = load_projects_by_group(2)
    ...
    with st.expander("새 프로젝트 추가"):
        ...
        with st.form("add_project_form"):
            ...
            # 신규 프로젝트 id 지정
            group_id = get_group_id_by_name("Team_2")
            if group_id is None:
                group_id = 2  # 혹은 기본값 지정
    ...
    df_project = st.session_state["projects_Team_2"]
