...
# Team_2 페이지 레이아웃 생성 함수
def get_Team_2_layout():
    # 초기 프로젝트 데이터 (Team_2은 group_id=2)
    projects_df = config.load_projects_by_group(2)
    tabs = []
    # 각 프로젝트에 대해 탭 생성
    for project in projects_df.to_dict('records'):
        ...
        tab = dcc.Tab(
            ...
            children=[
                html.Div([
                    html.Button("프로젝트 삭제", id={'type': 'team2-delete-project-btn', 'index': project_id}, n_clicks=0, style={'float': 'right'}),
                    html.Div([
                        html.H3("Main Task 관리"),
                        dash_table.DataTable(
                            id={'type': 'team2-main-task-table', 'index': project_id},
                            columns=[
                        ...
                        html.Button("Main Task 저장", id={'type': 'team2-save-main-task-btn', 'index': project_id}, n_clicks=0),
                        html.Div(id={'type': 'team2-main-task-message', 'index': project_id})
                    ], style={'width': '45%', 'display': 'inline-block', 'verticalAlign': 'top'}),
                    html.Div([
                        html.H3("Sub Task 관리"),
                        dash_table.DataTable(
                            id={'type': 'team2-sub-task-table', 'index': project_id},
                            columns=[
                        ...
                        html.Button("Sub Task 저장", id={'type': 'team2-save-sub-task-btn', 'index': project_id}, n_clicks=0),
                        html.Div(id={'type': 'team2-sub-task-message', 'index': project_id})
    ...
    layout = html.Div([
        ...
        html.Details([
            ...
            html.Div([
                html.Label("프로젝트 이름"),
                dcc.Input(id="team2-new-project-name", type="text", value="", style={'margin': '5px'}),
                html.Label("시작 날짜"),
                dcc.DatePickerSingle(id="team2-new-start-date", date=datetime.now().date()),
                html.Label("종료 날짜"),
                dcc.DatePickerSingle(id="team2-new-end-date", date=datetime.now().date()),
                html.Button("프로젝트 추가", id="team2-add-project-btn", n_clicks=0)
            ], style={'margin': '10px'})
        ]),
        html.Div(id="team2-add-project-message"),
        dcc.Tabs(id="team2-project-tabs", value=tabs[0].value if tabs else 'none', children=tabs),
        dcc.Store(id="team2-project-data", data=projects_df.to_dict('records'))
    ])
    return layout
